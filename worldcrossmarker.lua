        ...
        for i = 0, hitCount, 1 do
            if (hitTimes[i] ~= nil and hitPositions[i] ~= nil and hitTypes[i] ~= nil) then
                ...
                if xHit ~= nil and yHit ~= nil then
                    local alpha;
                    if (deltaTime > 2 / 2) then
                        alpha = (1 - (deltaTime - deltaTimes[i]) / 2 * 2) * 255;
                        if (alpha < 0) then
                            alpha = 0
                        end
                    else
                    ...
                    if (hitTypes[i] == 1) then
                        draw.Color(242, 48, 48, alpha)
                        
                        draw.FilledRect( xHit - 1, yHit - 5, xHit + 1, yHit + 5 )
                        draw.FilledRect( xHit - 5, yHit - 1, xHit + 5, yHit + 1 )
                    else
                        draw.Color(42, 235, 45, alpha)
